...
<?php if ( get_comment_type() == "comment" ) : ?>
						<li id="comment-<?php comment_ID() ?>" class="<?php sandbox_comment_class() ?>">
                            ...
                            <div class="comment-meta"><div class="comment-meta-author"><?php printf(__('Posted by %1$s</div><div class=\"comment-meta-date\">%2$s at %3$s <span class="meta-sep">|</span> <a href="%4$s" title="Permalink to this comment">Permalink</a>', 'sandbox'),
                                        get_comment_author_link(),
										get_comment_date(),
										get_comment_time(),
										'#comment-' . get_comment_ID() );
										edit_comment_link(__('Edit', 'sandbox'), ' <span class="meta-sep">|</span> <span class="edit-link">', '</span>'); ?></div></div>
...
<?php $req = get_option('require_name_email'); // Checks if fields are required. Thanks, Adam. ;-) ?>
				...
				<div id="respond">
					<h3><?php _e( 'Leave a Comment', 'sandbox' ) ?></h3>
...
<?php endif // REFERENCE: * if ( $user_ID ) ?>
							...
							<div class="form-submit"><input type="hidden" name="submit" value="<?php _e( 'Post Comment', 'sandbox' ) ?>" /><?php _e( 'Post Comment', 'sandbox' ) ?><!--span id="commentform_submit"--><input id="submit" name="submit" class="button" type="submit" value="<?php _e( 'Post Comment', 'sandbox' ) ?>" tabindex="7" /><!--/span--><input type="hidden" name="comment_post_ID" value="<?php echo $id ?>" /></div>
